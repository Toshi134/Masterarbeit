...
# Skript 09.4: Spezifische Verbindungen analysieren
# ============================================
...
for(cope_var in var_lists$cope_t4) {
  ...
  edges <- get_edges_between_groups(edge_matrix, cope_var, var_lists$stress_t4)
...
posr_rank <- all_coping_stressors %>% filter(Coping_Strategy == "bcope.posr.t4") %>% pull(Rank_Negativity)
acc_rank <- all_coping_stressors %>% filter(Coping_Strategy == "bcope.acc.t4") %>% pull(Rank_Negativity)
