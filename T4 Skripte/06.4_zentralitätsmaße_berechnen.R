...
# Skript 06.4: Zentralitätsmaße berechnen
...
cent_T4 <- centrality(network_T4)

# Als Dataframe - KORRIGIERT
cent_table <- data.frame(
  Node = names(cent_T4$OutDegree),  # names() statt rownames()
  Strength = as.numeric(cent_T4$OutDegree),
  Betweenness = as.numeric(cent_T4$Betweenness),
  Closeness = as.numeric(cent_T4$Closeness),
...
cent_table <- cent_table %>%
  mutate(
    Group = case_when(
      Node %in% var_lists$adnm_t4 ~ "ADNM-8",
      Node %in% var_lists$cope_t4 ~ "Coping",
      Node %in% var_lists$stress_t4 ~ "Stressoren",
      TRUE ~ "Unknown"
    )
  )
...
coping_display <- coping_cent %>%
  mutate(
    Label = case_when(
      Node == "bcope.posr.t4" ~ "Positive Reframing (H1a, H1c)",
      Node == "bcope.acc.t4" ~ "Acceptance (H1a)",
      Node == "bcope.behd.t4" ~ "Behavioral Disengagement (H1b)",
      Node == "bcope.sub.t4" ~ "Substance Use (explorativ)",
      Node == "bcope.act.t4" ~ "Active Coping",
      Node == "bcope.den.t4" ~ "Denial",
      Node == "bcope.dist.t4" ~ "Self-Distraction",
      Node == "bcope.esupp.t4" ~ "Emotional Support",
      Node == "bcope.hu.t4" ~ "Humor",
      Node == "bcope.isupp.t4" ~ "Instrumental Support",
      Node == "bcope.pl.t4" ~ "Planning",
      Node == "bcope.rel.t4" ~ "Religion",
      Node == "bcope.sbl.t4" ~ "Self-Blame",
      Node == "bcope.ven.t4" ~ "Venting",
      TRUE ~ Node
    )
  ) %>%
...
pos_refr_rank <- coping_cent %>% filter(Node == "bcope.posr.t4") %>% pull(Rank)
...
# H1a: Akzeptanz
acc_rank <- coping_cent %>% filter(Node == "bcope.acc.t4") %>% pull(Rank)
cat("\nH1a - Acceptance: Rang", acc_rank, "von 14\n")

# H1b: Behavioral Disengagement
behd_rank <- coping_cent %>% filter(Node == "bcope.behd.t4") %>% pull(Rank)
cat("\nH1b - Behavioral Disengagement: Rang", behd_rank, "von 14\n")

cat("\n--- EXPLORATIV: SUBSTANZKONSUM ---\n\n")

# Substanzkonsum explorativ
sub_rank <- coping_cent %>% filter(Node == "bcope.sub.t4") %>% pull(Rank)
sub_strength <- coping_cent %>% filter(Node == "bcope.sub.t4") %>% pull(Strength)
...
stressor_display <- stressor_cent %>%
  mutate(
    Label = case_when(
      Node == "bc.1.t4" ~ "Social Isolation",
      Node == "bc.2.t4" ~ "Restricted Contact Loved Ones",
      Node == "bc.5.t4" ~ "Unable to Visit Critical Ill",
      Node == "bh.1.t4" ~ "Fear of Infection",
      Node == "bh.6.t4" ~ "Fear Loved Ones Infected",
      Node == "bh.8.t4" ~ "Death of Loved One",
      Node == "bl.1.t4" ~ "Restricted Activity",
      Node == "bl.7.t4" ~ "Loss of Daily Structure",
      Node == "bp.1.t4" ~ "Uncertainty Pandemic",
      Node == "bp.3.t4" ~ "Poor Crisis Management",
      Node == "bs.4.t4" ~ "No Place of Retreat",
      Node == "bs.5.t4" ~ "Conflicts at Home",
      Node == "bw.4.t4" ~ "Income Loss",
      Node == "bw.5.t4" ~ "Job Loss",
      Node == "bw.7.t4" ~ "Increased Workload",
      TRUE ~ Node
    ),
    Category = case_when(
...
# Income Loss
income_rank <- stressor_cent %>% filter(Node == "bw.4.t4") %>% pull(Rank)
income_strength <- stressor_cent %>% filter(Node == "bw.4.t4") %>% pull(Strength)
cat("Income Loss: Rang", income_rank, "von 15 Stressoren\n")
cat("  Strength:", round(income_strength, 3), "\n")

# Job Loss
job_rank <- stressor_cent %>% filter(Node == "bw.5.t4") %>% pull(Rank)
job_strength <- stressor_cent %>% filter(Node == "bw.5.t4") %>% pull(Strength)
cat("\nJob Loss: Rang", job_rank, "von 15 Stressoren\n")
cat("  Strength:", round(job_strength, 3), "\n")

# Workload
work_rank <- stressor_cent %>% filter(Node == "bw.7.t4") %>% pull(Rank)
work_strength <- stressor_cent %>% filter(Node == "bw.7.t4") %>% pull(Strength)
...
# Soziale Isolation
social_rank <- stressor_cent %>% filter(Node == "bc.1.t4") %>% pull(Rank)
social_strength <- stressor_cent %>% filter(Node == "bc.1.t4") %>% pull(Strength)
...
pdf("plots/centrality_T4_all.pdf", width = 12, height = 10)
centralityPlot(network_T4, 
               include = c("Strength", "Betweenness", "Closeness"),
               orderBy = "Strength",
               scale = "z-scores")
dev.off()
cat("✓ Gespeichert: plots/centrality_T4_all.pdf\n")

# Nur Strength
pdf("plots/centrality_T4_strength.pdf", width = 8, height = 12)
centralityPlot(network_T4, 
               include = "Strength",
               orderBy = "Strength",
               scale = "raw")
dev.off()
cat("✓ Gespeichert: plots/centrality_T4_strength.pdf\n")

# Strength mit Gruppen-Farben
png("plots/centrality_T4_strength_colored.png", 
...
dev.off()
cat("✓ Gespeichert: plots/centrality_T4_strength_colored.png\n")
...
write.csv(coping_export, "results/centrality_coping_T4.csv", row.names = FALSE)
cat("✓ Coping-Tabelle: results/centrality_coping_T4.csv\n")
...
write.csv(stressor_export, "results/centrality_stressors_T4.csv", row.names = FALSE)
cat("✓ Stressor-Tabelle: results/centrality_stressors_T4.csv\n")

# Vollständige Tabelle
write.csv(cent_table, "results/centrality_T4_complete.csv", row.names = FALSE)
cat("✓ Vollständige Tabelle: results/centrality_T4_complete.csv\n")
