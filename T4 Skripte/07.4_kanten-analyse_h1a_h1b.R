...
# Skript 07.4: Spezifische Verbindungen analysieren
# ============================================
...
for(cope_var in var_lists$cope_t4) {
  
  edges <- get_edges_between_groups(edge_matrix, cope_var, var_lists$adnm_t4)
...
all_coping_adnm <- all_coping_adnm %>%
  mutate(
    Label = case_when(
      Coping_Strategy == "bcope.posr.t4" ~ "Positive Reframing",
      Coping_Strategy == "bcope.acc.t4" ~ "Acceptance",
      Coping_Strategy == "bcope.behd.t4" ~ "Behavioral Disengagement",
      Coping_Strategy == "bcope.sub.t4" ~ "Substance Use",
      Coping_Strategy == "bcope.act.t4" ~ "Active Coping",
      Coping_Strategy == "bcope.den.t4" ~ "Denial",
      Coping_Strategy == "bcope.dist.t4" ~ "Self-Distraction",
      Coping_Strategy == "bcope.esupp.t4" ~ "Emotional Support",
      Coping_Strategy == "bcope.hu.t4" ~ "Humor",
      Coping_Strategy == "bcope.isupp.t4" ~ "Instrumental Support",
      Coping_Strategy == "bcope.pl.t4" ~ "Planning",
      Coping_Strategy == "bcope.rel.t4" ~ "Religion",
      Coping_Strategy == "bcope.sbl.t4" ~ "Self-Blame",
      Coping_Strategy == "bcope.ven.t4" ~ "Venting",
...
posr_rank <- all_coping_adnm %>% filter(Coping_Strategy == "bcope.posr.t4") %>% pull(Rank_Negativity)
acc_rank <- all_coping_adnm %>% filter(Coping_Strategy == "bcope.acc.t4") %>% pull(Rank_Negativity)
...
behd_rank <- all_coping_adnm_pos %>% filter(Coping_Strategy == "bcope.behd.t4") %>% pull(Rank_Positivity)
