...
for(stress_var in var_lists$stress_t1) {
  
  edges <- get_edges_between_groups(edge_matrix, stress_var, var_lists$adnm_t1)
...
all_stress_adnm <- all_stress_adnm %>%
  mutate(
    Label = case_when(
      Stressor == "bc.1.t1" ~ "Social Isolation",
      Stressor == "bc.2.t1" ~ "Restricted Contact Loved Ones",
      Stressor == "bc.5.t1" ~ "Unable to Visit Critical Ill",
      Stressor == "bh.1.t1" ~ "Fear of Infection",
      Stressor == "bh.6.t1" ~ "Fear Loved Ones Infected",
      Stressor == "bh.8.t1" ~ "Death of Loved One",
      Stressor == "bl.1.t1" ~ "Restricted Activity",
      Stressor == "bl.7.t1" ~ "Loss of Daily Structure",
      Stressor == "bp.1.t1" ~ "Uncertainty Pandemic",
      Stressor == "bp.3.t1" ~ "Poor Crisis Management",
      Stressor == "bs.4.t1" ~ "No Place of Retreat",
      Stressor == "bs.5.t1" ~ "Conflicts at Home",
      Stressor == "bw.4.t1" ~ "Income Loss",
      Stressor == "bw.5.t1" ~ "Job Loss",
      Stressor == "bw.7.t1" ~ "Increased Workload",
      TRUE ~ Stressor
    ),
    Category = case_when(
