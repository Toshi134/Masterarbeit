...
cope_indices <- which(colnames(edge_matrix) %in% var_lists$cope_t1)
...
walktrap_membership <- data.frame(
  ...
  mutate(
    Label = case_when(
      Strategy == "bcope.acc.t1" ~ "Acceptance",
      Strategy == "bcope.act.t1" ~ "Active Coping",
      Strategy == "bcope.behd.t1" ~ "Behavioral Disengagement",
      Strategy == "bcope.den.t1" ~ "Denial",
      Strategy == "bcope.dist.t1" ~ "Self-Distraction",
      Strategy == "bcope.esupp.t1" ~ "Emotional Support",
      Strategy == "bcope.hu.t1" ~ "Humor",
      Strategy == "bcope.isupp.t1" ~ "Instrumental Support",
      Strategy == "bcope.pl.t1" ~ "Planning",
      Strategy == "bcope.posr.t1" ~ "Positive Reframing",
      Strategy == "bcope.rel.t1" ~ "Religion",
      Strategy == "bcope.sbl.t1" ~ "Self-Blame",
      Strategy == "bcope.sub.t1" ~ "Substance Use",
      Strategy == "bcope.ven.t1" ~ "Venting",
      TRUE ~ Strategy
    )
  ) %>%
...
for(i in unique(cluster_assignment$Cluster)) {
  ...
  for(strat in member_vars) {
    for(adnm_var in var_lists$adnm_t1) {
