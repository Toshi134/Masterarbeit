...
# Skript 10.5: Funktionale Coping-Cluster (H4a)
# ============================================
...
cope_indices <- which(colnames(edge_matrix) %in% var_lists$cope_t5)
...
walktrap_membership <- data.frame(
  ...
  mutate(
    Label = case_when(
      Strategy == "bcope.acc.t5" ~ "Acceptance",
      Strategy == "bcope.act.t5" ~ "Active Coping",
      Strategy == "bcope.behd.t5" ~ "Behavioral Disengagement",
      Strategy == "bcope.den.t5" ~ "Denial",
      Strategy == "bcope.dist.t5" ~ "Self-Distraction",
      Strategy == "bcope.esupp.t5" ~ "Emotional Support",
      Strategy == "bcope.hu.t5" ~ "Humor",
      Strategy == "bcope.isupp.t5" ~ "Instrumental Support",
      Strategy == "bcope.pl.t5" ~ "Planning",
      Strategy == "bcope.posr.t5" ~ "Positive Reframing",
      Strategy == "bcope.rel.t5" ~ "Religion",
      Strategy == "bcope.sbl.t5" ~ "Self-Blame",
      Strategy == "bcope.sub.t5" ~ "Substance Use",
      Strategy == "bcope.ven.t5" ~ "Venting",
      TRUE ~ Strategy
    )
  ) %>%
...
for(i in unique(cluster_assignment$Cluster)) {
  ...
  for(strat in member_vars) {
    for(adnm_var in var_lists$adnm_t5) {
