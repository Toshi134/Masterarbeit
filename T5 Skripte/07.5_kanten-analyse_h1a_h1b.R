...
# Skript 07.5: Spezifische Verbindungen analysieren
# ============================================
...
for(cope_var in var_lists$cope_t5) {
  
  edges <- get_edges_between_groups(edge_matrix, cope_var, var_lists$adnm_t5)
...
all_coping_adnm <- all_coping_adnm %>%
  mutate(
    Label = case_when(
      Coping_Strategy == "bcope.posr.t5" ~ "Positive Reframing",
      Coping_Strategy == "bcope.acc.t5" ~ "Acceptance",
      Coping_Strategy == "bcope.behd.t5" ~ "Behavioral Disengagement",
      Coping_Strategy == "bcope.sub.t5" ~ "Substance Use",
      Coping_Strategy == "bcope.act.t5" ~ "Active Coping",
      Coping_Strategy == "bcope.den.t5" ~ "Denial",
      Coping_Strategy == "bcope.dist.t5" ~ "Self-Distraction",
      Coping_Strategy == "bcope.esupp.t5" ~ "Emotional Support",
      Coping_Strategy == "bcope.hu.t5" ~ "Humor",
      Coping_Strategy == "bcope.isupp.t5" ~ "Instrumental Support",
      Coping_Strategy == "bcope.pl.t5" ~ "Planning",
      Coping_Strategy == "bcope.rel.t5" ~ "Religion",
      Coping_Strategy == "bcope.sbl.t5" ~ "Self-Blame",
      Coping_Strategy == "bcope.ven.t5" ~ "Venting",
...
posr_rank <- all_coping_adnm %>% filter(Coping_Strategy == "bcope.posr.t5") %>% pull(Rank_Negativity)
acc_rank <- all_coping_adnm %>% filter(Coping_Strategy == "bcope.acc.t5") %>% pull(Rank_Negativity)
...
behd_rank <- all_coping_adnm_pos %>% filter(Coping_Strategy == "bcope.behd.t5") %>% pull(Rank_Positivity)
