...
boot_edges_t5 <- bootnet(network_T5,
...
saveRDS(boot_edges_t5, "boot_edges_t5.rds")

# Edge Weight Stability plotten
pdf("bootstrap_edges_t5.pdf", width = 12, height = 8)
plot(boot_edges_t5, 
     labels = TRUE, 
     order = "sample",
     plot = "interval")
...
pdf("bootstrap_edge_differences_t5.pdf", width = 14, height = 10)
plot(boot_edges_t5, 
     plot = "difference",
...
set.seed(4140)
boot_case_t5 <- bootnet(network_T5,
                        nBoots = 2500,
                        type = "case",
...
# Ergebnisse speichern
saveRDS(boot_case_t5, "boot_case_t5.rds")

# Stability Plot erstellen
pdf("case_dropping_stability_t5.pdf", width = 12, height = 8)
plot(boot_case_t5, 
...
pdf("centrality_differences_strength_t5.pdf", width = 14, height = 10)
plot(boot_edges_t5,  # ← Nutze boot_edges_t5 statt boot_case_t5!
     plot = "difference",
     statistics = "strength",
     order = "sample")
dev.off()
...
for(i in 1:n_boots) {
  ...
  boot_data <- daten_T5_complete[boot_indices, ]
...
original_metrics <- compute_network_metrics(daten_T5_complete)
...
# Speichern
write.csv(structural_summary, "structural_metrics_bootstrap_t5.csv", row.names = FALSE)
saveRDS(structural_boots, "structural_boots_t5.rds")

# Visualisierung der Bootstrap-Verteilungen
pdf("structural_metrics_distributions_t5.pdf", width = 12, height = 8)
par(mfrow = c(2, 3))
...
sink("bootstrap_summary_t5.txt")
cat("=============================================================================\n")
cat("BOOTSTRAP-ANALYSE MESSZEITPUNKT 1\n")
cat("=============================================================================\n\n")

cat("Stichprobengröße:", nrow(daten_T5_complete), "\n")
cat("Anzahl Variablen:", ncol(daten_T5_complete), "\n")
cat("Bootstrap-Iterationen (Kanten):", boot_edges_t5$nBoots, "\n")
cat("Bootstrap-Iterationen (Case-Dropping):", boot_case_t5$nBoots, "\n")
cat("Bootstrap-Iterationen (Struktur):", n_boots, "\n\n")

cat("-----------------------------------------------------------------------------\n")
cat("1. KANTENSTABILITÄT\n")
cat("-----------------------------------------------------------------------------\n")
cat("Siehe Plots: bootstrap_edges_t5.pdf und bootstrap_edge_differences_t5.pdf\n\n")
...
cat("\nErstelte Dateien:\n")
cat("- boot_edges_t5.rds (Kantenstabilität)\n")
cat("- boot_case_t5.rds (Case-Dropping)\n")
cat("- structural_boots_t5.rds (Strukturelle Maße)\n")
cat("- Diverse PDF-Plots\n")
cat("- bootstrap_summary_t5.txt (Gesamtbericht)\n")
cat("- cs_coefficients_t5.txt (CS-Koeffizienten)\n")
cat("- structural_metrics_bootstrap_t5.csv (Strukturmaße)\n")
