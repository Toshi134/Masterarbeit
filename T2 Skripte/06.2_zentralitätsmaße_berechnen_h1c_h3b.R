...
cent_T2 <- centrality(network_T2)

# Als Dataframe - KORRIGIERT
cent_table <- data.frame(
  Node = names(cent_T2$OutDegree),  # names() statt rownames()
  Strength = as.numeric(cent_T2$OutDegree),
  Betweenness = as.numeric(cent_T2$Betweenness),
  Closeness = as.numeric(cent_T2$Closeness),
...
cent_table <- cent_table %>%
  mutate(
    Group = case_when(
      Node %in% var_lists$adnm_t2 ~ "ADNM-8",
      Node %in% var_lists$cope_t2 ~ "Coping",
      Node %in% var_lists$stress_t2 ~ "Stressoren",
      TRUE ~ "Unknown"
    )
  )
...
coping_display <- coping_cent %>%
  mutate(
    Label = case_when(
      Node == "bcope.posr.t2" ~ "Positive Reframing (H1a, H1c)",
      Node == "bcope.acc.t2" ~ "Acceptance (H1a)",
      Node == "bcope.behd.t2" ~ "Behavioral Disengagement (H1b)",
      Node == "bcope.sub.t2" ~ "Substance Use (explorativ)",
      Node == "bcope.act.t2" ~ "Active Coping",
      Node == "bcope.den.t2" ~ "Denial",
      Node == "bcope.dist.t2" ~ "Self-Distraction",
      Node == "bcope.esupp.t2" ~ "Emotional Support",
      Node == "bcope.hu.t2" ~ "Humor",
      Node == "bcope.isupp.t2" ~ "Instrumental Support",
      Node == "bcope.pl.t2" ~ "Planning",
      Node == "bcope.rel.t2" ~ "Religion",
      Node == "bcope.sbl.t2" ~ "Self-Blame",
      Node == "bcope.ven.t2" ~ "Venting",
      TRUE ~ Node
    )
  ) %>%
...
pos_refr_rank <- coping_cent %>% filter(Node == "bcope.posr.t2") %>% pull(Rank)
...
# H1a: Akzeptanz
acc_rank <- coping_cent %>% filter(Node == "bcope.acc.t2") %>% pull(Rank)
cat("\nH1a - Acceptance: Rang", acc_rank, "von 14\n")

# H1b: Behavioral Disengagement
behd_rank <- coping_cent %>% filter(Node == "bcope.behd.t2") %>% pull(Rank)
cat("\nH1b - Behavioral Disengagement: Rang", behd_rank, "von 14\n")

cat("\n--- EXPLORATIV: SUBSTANZKONSUM ---\n\n")

# Substanzkonsum explorativ
sub_rank <- coping_cent %>% filter(Node == "bcope.sub.t2") %>% pull(Rank)
sub_strength <- coping_cent %>% filter(Node == "bcope.sub.t2") %>% pull(Strength)
...
stressor_display <- stressor_cent %>%
  mutate(
    Label = case_when(
      Node == "bc.1.t2" ~ "Social Isolation",
      Node == "bc.2.t2" ~ "Restricted Contact Loved Ones",
      Node == "bc.5.t2" ~ "Unable to Visit Critical Ill",
      Node == "bh.1.t2" ~ "Fear of Infection",
      Node == "bh.6.t2" ~ "Fear Loved Ones Infected",
      Node == "bh.8.t2" ~ "Death of Loved One",
      Node == "bl.1.t2" ~ "Restricted Activity",
      Node == "bl.7.t2" ~ "Loss of Daily Structure",
      Node == "bp.1.t2" ~ "Uncertainty Pandemic",
      Node == "bp.3.t2" ~ "Poor Crisis Management",
      Node == "bs.4.t2" ~ "No Place of Retreat",
      Node == "bs.5.t2" ~ "Conflicts at Home",
      Node == "bw.4.t2" ~ "Income Loss",
      Node == "bw.5.t2" ~ "Job Loss",
      Node == "bw.7.t2" ~ "Increased Workload",
      TRUE ~ Node
    ),
    Category = case_when(
...
# Income Loss
income_rank <- stressor_cent %>% filter(Node == "bw.4.t2") %>% pull(Rank)
income_strength <- stressor_cent %>% filter(Node == "bw.4.t2") %>% pull(Strength)
cat("Income Loss: Rang", income_rank, "von 15 Stressoren\n")
cat("  Strength:", round(income_strength, 3), "\n")

# Job Loss
job_rank <- stressor_cent %>% filter(Node == "bw.5.t2") %>% pull(Rank)
job_strength <- stressor_cent %>% filter(Node == "bw.5.t2") %>% pull(Strength)
cat("\nJob Loss: Rang", job_rank, "von 15 Stressoren\n")
cat("  Strength:", round(job_strength, 3), "\n")

# Workload
work_rank <- stressor_cent %>% filter(Node == "bw.7.t2") %>% pull(Rank)
work_strength <- stressor_cent %>% filter(Node == "bw.7.t2") %>% pull(Strength)
...
# Soziale Isolation
social_rank <- stressor_cent %>% filter(Node == "bc.1.t2") %>% pull(Rank)
social_strength <- stressor_cent %>% filter(Node == "bc.1.t2") %>% pull(Strength)
...
pdf("plots/centrality_T2_all.pdf", width = 12, height = 10)
centralityPlot(network_T2, 
               include = c("Strength", "Betweenness", "Closeness"),
               orderBy = "Strength",
               scale = "z-scores")
dev.off()
cat("✓ Gespeichert: plots/centrality_T2_all.pdf\n")

# Nur Strength
pdf("plots/centrality_T2_strength.pdf", width = 8, height = 12)
centralityPlot(network_T2, 
               include = "Strength",
               orderBy = "Strength",
               scale = "raw")
dev.off()
cat("✓ Gespeichert: plots/centrality_T2_strength.pdf\n")

# Strength mit Gruppen-Farben
png("plots/centrality_T2_strength_colored.png", 
...
dev.off()
cat("✓ Gespeichert: plots/centrality_T2_strength_colored.png\n")
...
write.csv(coping_export, "results/centrality_coping_T2.csv", row.names = FALSE)
cat("✓ Coping-Tabelle: results/centrality_coping_T2.csv\n")
...
write.csv(stressor_export, "results/centrality_stressors_T2.csv", row.names = FALSE)
cat("✓ Stressor-Tabelle: results/centrality_stressors_T2.csv\n")

# Vollständige Tabelle
write.csv(cent_table, "results/centrality_T2_complete.csv", row.names = FALSE)
cat("✓ Vollständige Tabelle: results/centrality_T2_complete.csv\n")
