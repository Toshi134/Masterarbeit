...
# Skript 10: Funktionale Coping-Cluster (H4a)
# ============================================
...
cope_indices <- which(colnames(edge_matrix) %in% var_lists$cope_t2)
...
walktrap_membership <- data.frame(
  ...
  mutate(
    Label = case_when(
      Strategy == "bcope.acc.t2" ~ "Acceptance",
      Strategy == "bcope.act.t2" ~ "Active Coping",
      Strategy == "bcope.behd.t2" ~ "Behavioral Disengagement",
      Strategy == "bcope.den.t2" ~ "Denial",
      Strategy == "bcope.dist.t2" ~ "Self-Distraction",
      Strategy == "bcope.esupp.t2" ~ "Emotional Support",
      Strategy == "bcope.hu.t2" ~ "Humor",
      Strategy == "bcope.isupp.t2" ~ "Instrumental Support",
      Strategy == "bcope.pl.t2" ~ "Planning",
      Strategy == "bcope.posr.t2" ~ "Positive Reframing",
      Strategy == "bcope.rel.t2" ~ "Religion",
      Strategy == "bcope.sbl.t2" ~ "Self-Blame",
      Strategy == "bcope.sub.t2" ~ "Substance Use",
      Strategy == "bcope.ven.t2" ~ "Venting",
      TRUE ~ Strategy
    )
  ) %>%
...
for(i in unique(cluster_assignment$Cluster)) {
  ...
  for(strat in member_vars) {
    for(adnm_var in var_lists$adnm_t2) {
