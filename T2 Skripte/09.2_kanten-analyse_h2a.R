...
for(cope_var in var_lists$cope_t2) {
  ...
  edges <- get_edges_between_groups(edge_matrix, cope_var, var_lists$stress_t2)
...
posr_rank <- all_coping_stressors %>% filter(Coping_Strategy == "bcope.posr.t2") %>% pull(Rank_Negativity)
acc_rank <- all_coping_stressors %>% filter(Coping_Strategy == "bcope.acc.t2") %>% pull(Rank_Negativity)
