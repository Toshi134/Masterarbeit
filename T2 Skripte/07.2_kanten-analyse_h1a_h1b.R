...
for(cope_var in var_lists$cope_t2) {
  
  edges <- get_edges_between_groups(edge_matrix, cope_var, var_lists$adnm_t2)
...
all_coping_adnm <- all_coping_adnm %>%
  mutate(
    Label = case_when(
      Coping_Strategy == "bcope.posr.t2" ~ "Positive Reframing",
      Coping_Strategy == "bcope.acc.t2" ~ "Acceptance",
      Coping_Strategy == "bcope.behd.t2" ~ "Behavioral Disengagement",
      Coping_Strategy == "bcope.sub.t2" ~ "Substance Use",
      Coping_Strategy == "bcope.act.t2" ~ "Active Coping",
      Coping_Strategy == "bcope.den.t2" ~ "Denial",
      Coping_Strategy == "bcope.dist.t2" ~ "Self-Distraction",
      Coping_Strategy == "bcope.esupp.t2" ~ "Emotional Support",
      Coping_Strategy == "bcope.hu.t2" ~ "Humor",
      Coping_Strategy == "bcope.isupp.t2" ~ "Instrumental Support",
      Coping_Strategy == "bcope.pl.t2" ~ "Planning",
      Coping_Strategy == "bcope.rel.t2" ~ "Religion",
      Coping_Strategy == "bcope.sbl.t2" ~ "Self-Blame",
      Coping_Strategy == "bcope.ven.t2" ~ "Venting",
...
posr_rank <- all_coping_adnm %>% filter(Coping_Strategy == "bcope.posr.t2") %>% pull(Rank_Negativity)
acc_rank <- all_coping_adnm %>% filter(Coping_Strategy == "bcope.acc.t2") %>% pull(Rank_Negativity)
...
behd_rank <- all_coping_adnm_pos %>% filter(Coping_Strategy == "bcope.behd.t2") %>% pull(Rank_Positivity)
...
pdf("plots/coping_adnm_connections.pdf", width = 12, height = 10)
