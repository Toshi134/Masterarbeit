...
for(stress_var in var_lists$stress_t2) {
  
  edges <- get_edges_between_groups(edge_matrix, stress_var, var_lists$adnm_t2)
...
all_stress_adnm <- all_stress_adnm %>%
  mutate(
    Label = case_when(
      Stressor == "bc.1.t2" ~ "Social Isolation",
      Stressor == "bc.2.t2" ~ "Restricted Contact Loved Ones",
      Stressor == "bc.5.t2" ~ "Unable to Visit Critical Ill",
      Stressor == "bh.1.t2" ~ "Fear of Infection",
      Stressor == "bh.6.t2" ~ "Fear Loved Ones Infected",
      Stressor == "bh.8.t2" ~ "Death of Loved One",
      Stressor == "bl.1.t2" ~ "Restricted Activity",
      Stressor == "bl.7.t2" ~ "Loss of Daily Structure",
      Stressor == "bp.1.t2" ~ "Uncertainty Pandemic",
      Stressor == "bp.3.t2" ~ "Poor Crisis Management",
      Stressor == "bs.4.t2" ~ "No Place of Retreat",
      Stressor == "bs.5.t2" ~ "Conflicts at Home",
      Stressor == "bw.4.t2" ~ "Income Loss",
      Stressor == "bw.5.t2" ~ "Job Loss",
      Stressor == "bw.7.t2" ~ "Increased Workload",
      TRUE ~ Stressor
    ),
    Category = case_when(
