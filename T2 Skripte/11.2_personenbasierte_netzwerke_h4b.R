...
# Skript 08: Symptom-Netzwerke nach Coping-Profil (H4b)
# ============================================
...
daten_T2_complete <- daten_T2_complete %>%
...
cat("  ADNM-Variablen:", class(daten_T2_complete[[var_lists$adnm_t2[1]]]), "\n")
cat("  Coping-Variablen:", class(daten_T2_complete[[var_lists$cope_t2[1]]]), "\n\n")
...
coping_data <- daten_T2_complete[, var_lists$cope_t2]
...
adaptive_strategies <- c(
  ...
  "bcope.isupp.t2",  # Instrumental Support
  "bcope.rel.t2",    # Religion
  "bcope.hu.t2"      # Humor
)

maladaptive_strategies <- c(
  "bcope.behd.t2",   # Behavioral Disengagement
  "bcope.den.t2",    # Denial
...
cat("  Adaptive Strategien gefunden:", 
    sum(adaptive_strategies %in% names(daten_T2_complete)), "von", 
    length(adaptive_strategies), "\n")
cat("  Maladaptive Strategien gefunden:", 
    sum(maladaptive_strategies %in% names(daten_T2_complete)), "von", 
    length(maladaptive_strategies), "\n\n")

# Berechne Summenscores
daten_T2_complete$adaptive_score <- rowMeans(
  daten_T2_complete[, adaptive_strategies], 
  na.rm = TRUE
)

daten_T2_complete$maladaptive_score <- rowMeans(
  daten_T2_complete[, maladaptive_strategies], 
  na.rm = TRUE
)

# Prüfe ob Scores berechnet wurden
cat("Adaptive Score - Range:", 
    round(min(daten_T2_complete$adaptive_score, na.rm = TRUE), 2), "bis",
    round(max(daten_T2_complete$adaptive_score, na.rm = TRUE), 2), "\n")
cat("Maladaptive Score - Range:", 
    round(min(daten_T2_complete$maladaptive_score, na.rm = TRUE), 2), "bis",
    round(max(daten_T2_complete$maladaptive_score, na.rm = TRUE), 2), "\n\n")

# Ratio: Adaptiv vs. Maladaptiv
daten_T2_complete$coping_ratio <- 
  daten_T2_complete$adaptive_score / 
  (daten_T2_complete$maladaptive_score + 0.01)

# Entferne Inf/NaN Werte
daten_T2_complete$coping_ratio[is.infinite(daten_T2_complete$coping_ratio)] <- NA
daten_T2_complete$coping_ratio[is.nan(daten_T2_complete$coping_ratio)] <- NA

# Entferne Personen mit fehlenden Ratio-Werten
daten_T2_complete <- daten_T2_complete %>%
  filter(!is.na(coping_ratio))

cat("Nach Bereinigung: n =", nrow(daten_T2_complete), "\n\n")

# Visualisierung der Verteilung
pdf("plots/coping_ratio_distribution.pdf", width = 10, height = 6)

hist(daten_T2_complete$coping_ratio,
     breaks = 50,
...
abline(v = median(daten_T2_complete$coping_ratio, na.rm = TRUE), 
       col = "#E74C3C", lwd = 2, lty = 2)

legend("topright", 
       legend = c(paste("Median =", 
                        round(median(daten_T2_complete$coping_ratio, na.rm = TRUE), 2))),
...
median_ratio <- median(daten_T2_complete$coping_ratio)

daten_T2_complete$coping_group_median <- ifelse(
  daten_T2_complete$coping_ratio > median_ratio,
  "Adaptive",
  "Maladaptive"
)

cat("Methode 1 - Median-Split:\n")
cat("  Adaptive Gruppe: n =", sum(daten_T2_complete$coping_group_median == "Adaptive"), "\n")
cat("  Maladaptive Gruppe: n =", sum(daten_T2_complete$coping_group_median == "Maladaptive"), "\n\n")

# Methode 2: Tertile (extremere Gruppen)
tertiles <- quantile(daten_T2_complete$coping_ratio, probs = c(1/3, 2/3))

daten_T2_complete$coping_group_tertile <- case_when(
  daten_T2_complete$coping_ratio <= tertiles[1] ~ "Maladaptive",
  daten_T2_complete$coping_ratio >= tertiles[2] ~ "Adaptive",
  TRUE ~ "Mixed"
)

cat("Methode 2 - Tertile (extreme Gruppen):\n")
cat("  Adaptive Gruppe: n =", sum(daten_T2_complete$coping_group_tertile == "Adaptive"), "\n")
cat("  Mixed Gruppe: n =", sum(daten_T2_complete$coping_group_tertile == "Mixed"), "\n")
cat("  Maladaptive Gruppe: n =", sum(daten_T2_complete$coping_group_tertile == "Maladaptive"), "\n\n")
...
daten_T2_complete$coping_group_kmeans <- ifelse(
  kmeans_result$cluster == which.max(
    tapply(daten_T2_complete$adaptive_score, kmeans_result$cluster, mean)
  ),
  "Adaptive",
  "Maladaptive"
)

cat("  Adaptive Gruppe: n =", sum(daten_T2_complete$coping_group_kmeans == "Adaptive"), "\n")
cat("  Maladaptive Gruppe: n =", sum(daten_T2_complete$coping_group_kmeans == "Maladaptive"), "\n\n")
...
# Nur ADNM-Daten
adnm_data <- daten_T2_complete[, var_lists$adnm_t2]
...
adaptive_group <- daten_T2_complete %>%
  filter(coping_group_tertile == "Adaptive")

maladaptive_group <- daten_T2_complete %>%
...
adnm_adaptive <- as.data.frame(lapply(
  adaptive_group[, var_lists$adnm_t2], 
  as.numeric
))

adnm_maladaptive <- as.data.frame(lapply(
  maladaptive_group[, var_lists$adnm_t2],
  as.numeric
))
...
group_characteristics <- daten_T2_complete %>%
  ...
  summarise(
    ...
    Mean_ADNM_Total = mean(rowMeans(across(all_of(var_lists$adnm_t2))), na.rm = TRUE)
  )
