...
for(stress_var in var_lists$stress_t3) {
  
  edges <- get_edges_between_groups(edge_matrix, stress_var, var_lists$adnm_t3)
...
all_stress_adnm <- all_stress_adnm %>%
  mutate(
    Label = case_when(
      Stressor == "bc.1.t3" ~ "Social Isolation",
      Stressor == "bc.2.t3" ~ "Restricted Contact Loved Ones",
      Stressor == "bc.5.t3" ~ "Unable to Visit Critical Ill",
      Stressor == "bh.1.t3" ~ "Fear of Infection",
      Stressor == "bh.6.t3" ~ "Fear Loved Ones Infected",
      Stressor == "bh.8.t3" ~ "Death of Loved One",
      Stressor == "bl.1.t3" ~ "Restricted Activity",
      Stressor == "bl.7.t3" ~ "Loss of Daily Structure",
      Stressor == "bp.1.t3" ~ "Uncertainty Pandemic",
      Stressor == "bp.3.t3" ~ "Poor Crisis Management",
      Stressor == "bs.4.t3" ~ "No Place of Retreat",
      Stressor == "bs.5.t3" ~ "Conflicts at Home",
      Stressor == "bw.4.t3" ~ "Income Loss",
      Stressor == "bw.5.t3" ~ "Job Loss",
      Stressor == "bw.7.t3" ~ "Increased Workload",
      TRUE ~ Stressor
    ),
    Category = case_when(
