...
cent_T3 <- centrality(network_T3)

# Als Dataframe - KORRIGIERT
cent_table <- data.frame(
  Node = names(cent_T3$OutDegree),  # names() statt rownames()
  Strength = as.numeric(cent_T3$OutDegree),
  Betweenness = as.numeric(cent_T3$Betweenness),
  Closeness = as.numeric(cent_T3$Closeness),
...
cent_table <- cent_table %>%
  mutate(
    Group = case_when(
      Node %in% var_lists$adnm_t3 ~ "ADNM-8",
      Node %in% var_lists$cope_t3 ~ "Coping",
      Node %in% var_lists$stress_t3 ~ "Stressoren",
      TRUE ~ "Unknown"
    )
  )
...
coping_display <- coping_cent %>%
  mutate(
    Label = case_when(
      Node == "bcope.posr.t3" ~ "Positive Reframing (H1a, H1c)",
      Node == "bcope.acc.t3" ~ "Acceptance (H1a)",
      Node == "bcope.behd.t3" ~ "Behavioral Disengagement (H1b)",
      Node == "bcope.sub.t3" ~ "Substance Use (explorativ)",
      Node == "bcope.act.t3" ~ "Active Coping",
      Node == "bcope.den.t3" ~ "Denial",
      Node == "bcope.dist.t3" ~ "Self-Distraction",
      Node == "bcope.esupp.t3" ~ "Emotional Support",
      Node == "bcope.hu.t3" ~ "Humor",
      Node == "bcope.isupp.t3" ~ "Instrumental Support",
      Node == "bcope.pl.t3" ~ "Planning",
      Node == "bcope.rel.t3" ~ "Religion",
      Node == "bcope.sbl.t3" ~ "Self-Blame",
      Node == "bcope.ven.t3" ~ "Venting",
      TRUE ~ Node
    )
  ) %>%
...
pos_refr_rank <- coping_cent %>% filter(Node == "bcope.posr.t3") %>% pull(Rank)
...
# H1a: Akzeptanz
acc_rank <- coping_cent %>% filter(Node == "bcope.acc.t3") %>% pull(Rank)
cat("\nH1a - Acceptance: Rang", acc_rank, "von 14\n")

# H1b: Behavioral Disengagement
behd_rank <- coping_cent %>% filter(Node == "bcope.behd.t3") %>% pull(Rank)
cat("\nH1b - Behavioral Disengagement: Rang", behd_rank, "von 14\n")

cat("\n--- EXPLORATIV: SUBSTANZKONSUM ---\n\n")

# Substanzkonsum explorativ
sub_rank <- coping_cent %>% filter(Node == "bcope.sub.t3") %>% pull(Rank)
sub_strength <- coping_cent %>% filter(Node == "bcope.sub.t3") %>% pull(Strength)
...
stressor_display <- stressor_cent %>%
  mutate(
    Label = case_when(
      Node == "bc.1.t3" ~ "Social Isolation",
      Node == "bc.2.t3" ~ "Restricted Contact Loved Ones",
      Node == "bc.5.t3" ~ "Unable to Visit Critical Ill",
      Node == "bh.1.t3" ~ "Fear of Infection",
      Node == "bh.6.t3" ~ "Fear Loved Ones Infected",
      Node == "bh.8.t3" ~ "Death of Loved One",
      Node == "bl.1.t3" ~ "Restricted Activity",
      Node == "bl.7.t3" ~ "Loss of Daily Structure",
      Node == "bp.1.t3" ~ "Uncertainty Pandemic",
      Node == "bp.3.t3" ~ "Poor Crisis Management",
      Node == "bs.4.t3" ~ "No Place of Retreat",
      Node == "bs.5.t3" ~ "Conflicts at Home",
      Node == "bw.4.t3" ~ "Income Loss",
      Node == "bw.5.t3" ~ "Job Loss",
      Node == "bw.7.t3" ~ "Increased Workload",
      TRUE ~ Node
    ),
    Category = case_when(
...
# Income Loss
income_rank <- stressor_cent %>% filter(Node == "bw.4.t3") %>% pull(Rank)
income_strength <- stressor_cent %>% filter(Node == "bw.4.t3") %>% pull(Strength)
cat("Income Loss: Rang", income_rank, "von 15 Stressoren\n")
cat("  Strength:", round(income_strength, 3), "\n")

# Job Loss
job_rank <- stressor_cent %>% filter(Node == "bw.5.t3") %>% pull(Rank)
job_strength <- stressor_cent %>% filter(Node == "bw.5.t3") %>% pull(Strength)
cat("\nJob Loss: Rang", job_rank, "von 15 Stressoren\n")
cat("  Strength:", round(job_strength, 3), "\n")

# Workload
work_rank <- stressor_cent %>% filter(Node == "bw.7.t3") %>% pull(Rank)
work_strength <- stressor_cent %>% filter(Node == "bw.7.t3") %>% pull(Strength)
...
# Soziale Isolation
social_rank <- stressor_cent %>% filter(Node == "bc.1.t3") %>% pull(Rank)
social_strength <- stressor_cent %>% filter(Node == "bc.1.t3") %>% pull(Strength)
...
pdf("plots/centrality_T3_all.pdf", width = 12, height = 10)
centralityPlot(network_T3, 
               include = c("Strength", "Betweenness", "Closeness"),
               orderBy = "Strength",
               scale = "z-scores")
dev.off()
cat("✓ Gespeichert: plots/centrality_T3_all.pdf\n")

# Nur Strength
pdf("plots/centrality_T3_strength.pdf", width = 8, height = 12)
centralityPlot(network_T3, 
               include = "Strength",
               orderBy = "Strength",
               scale = "raw")
dev.off()
cat("✓ Gespeichert: plots/centrality_T3_strength.pdf\n")

# Strength mit Gruppen-Farben
png("plots/centrality_T3_strength_colored.png", 
...
dev.off()
cat("✓ Gespeichert: plots/centrality_T3_strength_colored.png\n")
...
write.csv(coping_export, "results/centrality_coping_T3.csv", row.names = FALSE)
cat("✓ Coping-Tabelle: results/centrality_coping_T3.csv\n")
...
write.csv(stressor_export, "results/centrality_stressors_T3.csv", row.names = FALSE)
cat("✓ Stressor-Tabelle: results/centrality_stressors_T3.csv\n")

# Vollständige Tabelle
write.csv(cent_table, "results/centrality_T3_complete.csv", row.names = FALSE)
cat("✓ Vollständige Tabelle: results/centrality_T3_complete.csv\n")
