...
for(cope_var in var_lists$cope_t3) {
  ...
  edges <- get_edges_between_groups(edge_matrix, cope_var, var_lists$stress_t3)
...
posr_rank <- all_coping_stressors %>% filter(Coping_Strategy == "bcope.posr.t3") %>% pull(Rank_Negativity)
acc_rank <- all_coping_stressors %>% filter(Coping_Strategy == "bcope.acc.t3") %>% pull(Rank_Negativity)
