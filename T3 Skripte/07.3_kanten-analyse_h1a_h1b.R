...
for(cope_var in var_lists$cope_t3) {
  
  edges <- get_edges_between_groups(edge_matrix, cope_var, var_lists$adnm_t3)
...
all_coping_adnm <- all_coping_adnm %>%
  mutate(
    Label = case_when(
      Coping_Strategy == "bcope.posr.t3" ~ "Positive Reframing",
      Coping_Strategy == "bcope.acc.t3" ~ "Acceptance",
      Coping_Strategy == "bcope.behd.t3" ~ "Behavioral Disengagement",
      Coping_Strategy == "bcope.sub.t3" ~ "Substance Use",
      Coping_Strategy == "bcope.act.t3" ~ "Active Coping",
      Coping_Strategy == "bcope.den.t3" ~ "Denial",
      Coping_Strategy == "bcope.dist.t3" ~ "Self-Distraction",
      Coping_Strategy == "bcope.esupp.t3" ~ "Emotional Support",
      Coping_Strategy == "bcope.hu.t3" ~ "Humor",
      Coping_Strategy == "bcope.isupp.t3" ~ "Instrumental Support",
      Coping_Strategy == "bcope.pl.t3" ~ "Planning",
      Coping_Strategy == "bcope.rel.t3" ~ "Religion",
      Coping_Strategy == "bcope.sbl.t3" ~ "Self-Blame",
      Coping_Strategy == "bcope.ven.t3" ~ "Venting",
...
posr_rank <- all_coping_adnm %>% filter(Coping_Strategy == "bcope.posr.t3") %>% pull(Rank_Negativity)
acc_rank <- all_coping_adnm %>% filter(Coping_Strategy == "bcope.acc.t3") %>% pull(Rank_Negativity)
...
behd_rank <- all_coping_adnm_pos %>% filter(Coping_Strategy == "bcope.behd.t3") %>% pull(Rank_Positivity)
