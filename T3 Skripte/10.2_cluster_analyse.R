...
cope_indices <- which(colnames(edge_matrix) %in% var_lists$cope_t3)
...
walktrap_membership <- data.frame(
  ...
  mutate(
    Label = case_when(
      Strategy == "bcope.acc.t3" ~ "Acceptance",
      Strategy == "bcope.act.t3" ~ "Active Coping",
      Strategy == "bcope.behd.t3" ~ "Behavioral Disengagement",
      Strategy == "bcope.den.t3" ~ "Denial",
      Strategy == "bcope.dist.t3" ~ "Self-Distraction",
      Strategy == "bcope.esupp.t3" ~ "Emotional Support",
      Strategy == "bcope.hu.t3" ~ "Humor",
      Strategy == "bcope.isupp.t3" ~ "Instrumental Support",
      Strategy == "bcope.pl.t3" ~ "Planning",
      Strategy == "bcope.posr.t3" ~ "Positive Reframing",
      Strategy == "bcope.rel.t3" ~ "Religion",
      Strategy == "bcope.sbl.t3" ~ "Self-Blame",
      Strategy == "bcope.sub.t3" ~ "Substance Use",
      Strategy == "bcope.ven.t3" ~ "Venting",
      TRUE ~ Strategy
    )
  ) %>%
...
for(i in unique(cluster_assignment$Cluster)) {
  ...
  for(strat in member_vars) {
    for(adnm_var in var_lists$adnm_t3) {
